...
function getAllOrders() {
  ...
  details.forEach((row, index) =>
    row.push(...required[index], ...status[index], ...progress[index])
  )
...
function makeWeeklySchedule() {
  ...
  weekly
    .getRange(7, 1, weekly.getMaxRows(), weekly.getMaxColumns())
    .removeCheckboxes()
    .clear()
...
function updateConsumerUnits() {
  ...
  let currentMaxSerial = consumerUnitsSheet
    .getRange(
      ...
      consumerUnitsSheet.getLastColumn()
    )
    .getValue()
  ...
  allOrders.forEach(batch => {
    const index = Utils.search(
      ...
      batch.batch,
      0,
      consumerUnitBatchCheck.length - 1
    )
...
function updateLiveBatches() {
  ...
  if (newLiveBatches.length) {
    liveBatchesSheet
      .getRange(2, 1, newLiveBatches.length, newLiveBatches[0].length)
      .setValues(newLiveBatches)
  }
