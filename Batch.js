class Batch {
    ...
    getFormattedDates() {
        ...
        const timestamp2Suffix = 'timestamp2' in this ? Batch.getSuffix(this.timestamp2.getDate()) : null

        if (!timestamp2Suffix) {
            return Utilities.formatDate(this.timestamp, 'GMT', `EEEE d'${timestampSuffix}' MMMM yyyy`)
        }

        return [
            Utilities.formatDate(this.timestamp, 'GMT', `   EEEE d'${timestampSuffix}' MMMM yyyy`),
            Utilities.formatDate(this.timestamp2, 'GMT', `   EEEE d'${timestamp2Suffix}' MMMM yyyy`),
        ]
    }
    ...
    getLine() {
        ...
        return start.includes('R') ? 'Line 3' : start.includes('H') ? 'Line 7' : null
    }
