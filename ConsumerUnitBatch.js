class ConsumerUnitBatch extends Batch {
    ...
    getNextSerials(currentMax) {
        if (!currentMax) {
            const sheet = spreadsheet.getSheetByName('Consumer Units')
            const currentMax = sheet.getRange(sheet.getLastRow(), sheet.getLastColumn()).getValue()
        }
    ...
    push(array) {
        ...
        array.push([this.batch, this.group, this.description, this.qty, this.model, this.serialMin, this.serialMax])
